...
WatchDog::WatchDog(Uint32 interval) : m_watchedCount(0) {
...
Uint32 WatchDog::setCheckInterval(Uint32 interval) {
  // An interval of less than 70ms is not acceptable
  return theInterval = (interval < 70 ? 70 : interval);
}

bool WatchDog::registerWatchedThread(Uint32 *counter, Uint32 threadId) {
  bool ret;
...
void WatchDog::unregisterWatchedThread(Uint32 threadId) {
  Uint32 i;
  NdbMutex_Lock(m_mutex);

  for (i = 0; i < m_watchedCount; i++) {
    if (threadId == m_watchedList[i].m_threadId) break;
  }
...
struct NdbThread *WatchDog::doStart() {
  theStop = false;
  theThreadPtr = NdbThread_Create(runWatchDog, (void **)this,
                                  0,  // default stack size
                                  "ndb_watchdog", NDB_THREAD_PRIO_HIGH);

  return theThreadPtr;
}

void WatchDog::doStop() {
  void *status;
  theStop = true;
  if (theThreadPtr) {
...
void WatchDog::setKillSwitch(bool kill) {
  g_eventLogger->info("Watchdog KillSwitch %s.", (kill ? "on" : "off"));
  killer = kill;
}

static const char *get_action(char *buf, Uint32 IPValue) {
  const char *action;
  ...
  switch (place) {
    case 1: {
      ...
      BaseString::snprintf(buf, 128, "JobHandling in block: %u, gsn: %u", bno,
                           gsn);
      action = buf;
      break;
    }
...
void WatchDog::run() {
  ...
  if (!NdbTick_IsMonotonic()) {
    ...
    g_eventLogger->warning(
        "Adjusting system time manually, or otherwise (e.g. NTP), "
    ...
    // Print warnings if sleeping much longer than expected
      ...
      if (times(&my_tms) != (clock_t)-1) {
        g_eventLogger->info("Watchdog: User time: %llu  System time: %llu",
                            (Uint64)my_tms.tms_utime, (Uint64)my_tms.tms_stime);
      } else {
        g_eventLogger->info(
            "Watchdog: User time: %llu System time: %llu (errno=%d)",
            (Uint64)my_tms.tms_utime, (Uint64)my_tms.tms_stime, errno);
      }
      g_eventLogger->warning(
          "Watchdog: Warning overslept %llu ms, expected %u ms.",
          NdbTick_Elapsed(last_ticks, now).milliSec(), sleep_time);
    }
    last_ticks = now;
      ...
      */
      if (oldCounterValue[i] != 9 || elapsed[i] >= theIntervalCheck[i]) {
        ...
        if (last_stuck_action != NULL) {
          ...
          g_eventLogger->warning(
              "Ndb kernel thread %u is stuck in: Unknown place %u "
              "elapsed=%u",
...
void WatchDog::shutdownSystem(const char *last_stuck_action) {
  ErrorReporter::handleError(NDBD_EXIT_WATCHDOG_TERMINATE, last_stuck_action,
                             __FILE__, NST_Watchdog);
}
