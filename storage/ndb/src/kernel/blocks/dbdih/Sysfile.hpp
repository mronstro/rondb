...
struct Sysfile {
 public:
  ...
  static void setNodeGroup_v1(NodeId nodeId, Uint32 *nodeGroups, Uint8 group);
...
inline bool Sysfile::getInitialStartOngoing() const {
  return systemRestartBits & 1;
}

inline void Sysfile::setInitialStartOngoing() { systemRestartBits |= 1; }

inline void Sysfile::clearInitialStartOngoing() { systemRestartBits &= ~1; }

inline bool Sysfile::getRestartOngoing() const {
  return (systemRestartBits & 2) != 0;
}

inline void Sysfile::setRestartOngoing() { systemRestartBits |= 2; }

inline void Sysfile::clearRestartOngoing() { systemRestartBits &= ~2; }

inline bool Sysfile::getLCPOngoing() const { return systemRestartBits & 4; }

inline void Sysfile::setLCPOngoing() { systemRestartBits |= 4; }

inline void Sysfile::clearLCPOngoing() { systemRestartBits &= ~4; }

inline Uint32 Sysfile::getNodeStatus(NodeId nodeId) const {
  return getNodeStatus_v1(nodeId, nodeStatus);
}

inline void Sysfile::setNodeStatus(NodeId nodeId, Uint32 status) {
  ...
  if (nodeId > maxNodeId && status != Sysfile::NS_NotDefined) {
    ...
    do {
      nodeId--;
    } while (nodeId > 0 && getNodeStatus(nodeId) == Sysfile::NS_NotDefined);
    maxNodeId = nodeId;
  }
}
...
inline Uint32 Sysfile::getNodeStatus_v1(NodeId nodeId,
...
inline void Sysfile::setNodeStatus_v1(NodeId nodeId, Uint32 status,
                                      Uint32 *nodeStatus) {
  const int word = nodeId >> 3;
  const int shift = (nodeId & 7) << 2;
...
inline NodeId Sysfile::getNodeGroup(NodeId nodeId) const {
  return nodeGroups[nodeId];
}

inline void Sysfile::setNodeGroup(NodeId nodeId, Uint16 group) {
  nodeGroups[nodeId] = group;
}

inline NodeId Sysfile::getTakeOverNode(NodeId nodeId) const {
  return takeOver[nodeId];
}

inline void Sysfile::setTakeOverNode(NodeId nodeId, NodeId toNode) {
  takeOver[nodeId] = toNode;
}

inline NodeId Sysfile::getNodeGroup_v1(NodeId nodeId,
...
inline void Sysfile::setNodeGroup_v1(NodeId nodeId, Uint32 *nodeGroups,
                                     Uint8 group) {
  const int word = nodeId >> 2;
  const int shift = (nodeId & 3) << 3;
...
inline NodeId Sysfile::getTakeOverNode_v1(NodeId nodeId,
...
inline void Sysfile::setTakeOverNode_v1(NodeId nodeId, Uint32 *takeOver,
                                        Uint8 toNode) {
  const int word = nodeId >> 2;
  const int shift = (nodeId & 3) << 3;
