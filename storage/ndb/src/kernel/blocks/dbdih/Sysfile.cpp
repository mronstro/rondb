...
int Sysfile::pack_sysfile_format_v2(Uint32 cdata[],
...
int Sysfile::pack_sysfile_format_v1(Uint32 cdata[],
  ...
  for (Uint32 i = 0; i < 49; i++) cdata[6 + i] = lastCompletedGCI[i];
  ...
  for (Uint32 i = 0; i < 2; i++) cdata[88 + i] = lcpActive[i];

  return 0;
}

int Sysfile::unpack_sysfile_format_v2(const Uint32 cdata[],
                                      Uint32 *cdata_size_ptr) {
  initSysFile();
  Uint32 index = 0;
  if (std::memcmp(&cdata[index], MAGIC_v2, MAGIC_SIZE_v2) != 0) {
    return -1;
  }
  index += MAGIC_SIZE_v2 / sizeof(Uint32);
  ...
  *cdata_size_ptr = cdata_size;
  ...
  // TODO YYY clear fields for unused nodes and node groups, or keep
  // max_node/max_node_group in state! nodeStatus[nodeId] = NS_NotDefined
  // nodeGroups[i] = NO_NODE_GROUP_ID
  return 0;
}

int Sysfile::unpack_sysfile_format_v1(const Uint32 cdata[],
                                      Uint32 *cdata_size_ptr) {
