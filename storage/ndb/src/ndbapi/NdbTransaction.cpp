...
class BlobBatchChecker {
    ...
    for (Uint32 t = 0; t < m_tables.size(); t++) {
      if (m_tables[t].m_tableId == tableId) {
        ...
        DBUG_PRINT("info", ("found tableId %u indexId %u",
                            m_tables[t].m_tableId, m_tables[t].m_indexId));
  ...
  void addIndexForTable(const Uint32 tableId, const Uint32 indexId) {
    ...
    DBUG_PRINT("info", ("Added tableId %u indexId %u as entry %u", tableId,
                        indexId, m_tables.size() - 1));
    DBUG_VOID_RETURN;
  }
  ...
  void setMultiIndexForTable(const Uint32 tableId) {
    ...
    for (Uint32 t = 0; t < m_tables.size(); t++) {
      if (m_tables[t].m_tableId == tableId) {
        DBUG_PRINT("info", ("Setting tableId %u index to Multi", tableId));
  ...
  void addKey(NdbBlob *blob) {
    ...
    DBUG_PRINT("info", ("Adding blob %p to bucket idx %u current blob %p", blob,
                        bucketIdx, m_buckets[bucketIdx]));
 ...
 public:
  BlobBatchChecker() : m_batchOpTypes(0), m_tables(10) {
  ...
  bool includeNextOpInBatch(NdbOperation *nextOp) {
    ...
    if (!nextOp->theBlobList) {
      ...
      DBUG_PRINT("info",
                 ("opType %u tableId %u indexId %u typeSafe %u "
                          "firstAccess %u singleIndex %u",
                  opType, tableId, indexId,
                  NdbBlob::isOpTypeSafeWithBatch(m_batchOpTypes, opType),
                  firstAccess, singleIndex));
      ...
      if (firstAccess) {
         ...
         * enough to proceed
         */
        if (NdbBlob::isOpTypeSafeWithBatch(m_batchOpTypes, opType)) {
      ...
      if (include) {
        ...
        /* Update tables list */
        if (firstAccess) {
          addIndexForTable(tableId, indexId);
        } else if (!singleIndex) {
...
Remark:        Sets an error code on the connection object from an 
               operation object. 
*****************************************************************************/
void NdbTransaction::setOperationErrorCode(int error) {
...
*****************************************************************************/
void NdbTransaction::handleExecuteCompletion() {
  /***************************************************************************
  ...
  if (tLastExecOp != nullptr) {
    ...
    if (theCompletedLastOp == nullptr) theCompletedLastOp = tLastExecOp;
    theFirstExecOpInList = nullptr;
    theLastExecOpInList = nullptr;
  }  // if
...
class OpList {
 ...
 public:
  ...
  void checkOpInList(const NdbOperation *op) {
    ...
    } while (lop != nullptr && lop != m_listLast &&
   ...
   */
  OpList(NdbOperation *&listFirst, NdbOperation *&listLast)
   ...
   * Anything saved attm
   */
  bool isEmpty() const { return (m_savedFirst == nullptr); }
  ...
  static bool allOpsHaveFlagsSet(const NdbOperation *opFirst,
                                 const NdbOperation *opLast, Uint8 flags) {
    if (opFirst) {
      assert(opLast);
...
Remark:        Initialise connection object for new transaction. 
*****************************************************************************/
int NdbTransaction::execute(ExecType aTypeOfExec,
  ...
  DBUG_PRINT("enter",
             ("aTypeOfExec: %d, abortOption: %d", aTypeOfExec, abortOption));
  ...
  if (abortOption != NdbOperation::DefaultAbortOption) {
    ...
    while (tPrepOp != nullptr) {
      DBUG_PRINT("info",
                 ("Changing abortOption from %d", tPrepOp->m_abortOption));
  ...
  ExecType tExecType;
  OpList completedOps(theCompletedFirstOp, theCompletedLastOp);

  BlobBatchChecker batchChecker;

  int ret = 0;
  do {
    OpList nextBatchOps(theFirstOpInList, theLastOpInList);
      ...
      if (tPrepOp->theError.code == 0) {
        ...
        if (tBlob != nullptr) {
          ...
          /* Prepare this operation + blob ops now */
          {
             ...
             * Blob preExecute
             */
            OpList precedingOps(theFirstOpInList, theLastOpInList);
            if (prevOp) {
             ...
             * before them
             */
            OpList followingOps(theFirstOpInList, theLastOpInList);
            ...
            while (tBlob != nullptr) {
              ...
              if (ba == NdbBlob::BA_ERROR) {
                ret = -1;
                if (firstTransError.code == 0) firstTransError = theError;
...

    {
      ...
      if (executeNoBlobs(tExecType, NdbOperation::DefaultAbortOption,
                         forceSend) == -1) {
        if (theError.code == 626) {
          ...
          if (OpList::allOpsHaveFlagsSet(execOpFirst, execOpLast,
        ...
        /* Capture any trans error left by the execute() in case it gets
         * trampled */
        if (firstTransError.code != 0) theError = firstTransError;
        DBUG_RETURN(-1);
      }
    }
    ...
    do {
      ...
      const bool exec_now = postexec_again ||  // More to do
      ...
      if (exec_now) {
        ...
        if (executeNoBlobs(NdbTransaction::NoCommit,
                           NdbOperation::DefaultAbortOption, forceSend) == -1) {
  ...
  if (theError.code == 4350) // Trans already aborted
  {
    DBUG_PRINT("info", ("Trans already aborted, existingTransError.code %u, "
                        ...
                        existingTransError.code, firstTransError.code));
  ...
  if (firstTransError.code != 0) {
    DBUG_PRINT("info",
               ("Setting error to first error.  firstTransError.code = %u, "
                        "theError.code = %u",
                firstTransError.code, theError.code));
    theError = firstTransError;
  }
...
int NdbTransaction::executeNoBlobs(NdbTransaction::ExecType aTypeOfExec,
			       NdbOperation::AbortOption abortOption,
                                   int forceSend) {
  ...
  if (m_waitForReply) {
    while (1) {
       ...
       */
      if (theListState != NotInList) continue;
#ifdef VM_TRACE
      unsigned anyway = 0;
...
*****************************************************************************/
void NdbTransaction::executeAsynchPrepare(
    NdbTransaction::ExecType aTypeOfExec, NdbAsynchCallback aCallback,
    void *anyObject, NdbOperation::AbortOption abortOption) {
   ...
   */
  if (theError.code != 4012) theError.code = 0;
  ...
  if ((tCommitStatus != Started) || (aTypeOfExec == Rollback)) {
    /*****************************************************************************
 ...
 ****************************************************************************/
  ...
  if (tTransactionIsStarted == true) {
    if (tLastOp != nullptr) {
      if (aTypeOfExec == Commit) {
        /*****************************************************************************
         *	Set commit indicator on last operation when commit has been
         *ordered and also a number of operations.
         ******************************************************************************/
  ...
  while (tOp) {
    ...
    if (tOp->Status() == NdbOperation::UseNdbRecord)
      ...
      tReturnCode =
          tOp->prepareSend(theTCConPtr, theTransactionId, abortOption);
...
void NdbTransaction::executeAsynch(ExecType aTypeOfExec,
                                   NdbAsynchCallback aCallback, void *anyObject,
                              NdbOperation::AbortOption abortOption,
                                   int forceSend) {
...
void NdbTransaction::close() { theNdb->closeTransaction(this); }
...
******************************************************************************/
int NdbTransaction::sendTC_HBREP()  // Send a TC_HBREP signal;
{
  ...
  */
  ...
  case sendOperations: {
    ...
    if (theFirstExecOpInList || lastLookupQuery != nullptr) {
      ...
      theSendStatus = sendCompleted;
        theNdb->insert_completed_list(
            this);  // Scans query completes after send
    }
    DBUG_RETURN(0);
    }  // case
  case sendABORT:
...
***************************************************************************/
int NdbTransaction::sendCOMMIT()  // Send a TC_COMMITREQ signal;
{
  NdbApiSignal tSignal(theNdb->theMyRef);
  Uint32 tTransId1, tTransId2;
  NdbImpl *impl = theNdb->theImpl;
...
void NdbTransaction::releaseOps(NdbOperation *tOp) {
  while (tOp != nullptr) {
...
Remark:         Release all operations.
******************************************************************************/
void NdbTransaction::releaseOperations() {
...
void NdbTransaction::releaseCompletedOperations() {
  releaseOps(theCompletedFirstOp);
  theCompletedFirstOp = nullptr;
  theCompletedLastOp = nullptr;
  theErrorLine = 0;
...
void NdbTransaction::releaseCompletedQueries() {
  /**
...
******************************************************************************/
void NdbTransaction::releaseQueries(NdbQueryImpl *query) {
  while (query != nullptr) {
    NdbQueryImpl *next = query->getNext();
    query->release();
    query = next;
...
void NdbTransaction::releaseLockHandles() {
...
Remark:          If the schema object ownership check is enabled while creating 
                 ...
                 connection. 
*****************************************************************************/
bool NdbTransaction::checkSchemaObjects(const NdbTableImpl *tab,
                                        const NdbIndexImpl *idx) {
  bool ret = true;
  if (m_enable_schema_obj_owner_check) {
    ...
    if (strcmp(db, old_db) != 0) change_db = true;
    if (change_db &&
        (strcmp(db, "") != 0))  // switch to db of current table if not blank
    ...
    if (change_db &&
        strcmp(old_db, "") != 0)  // restore original value of db if not blank
...
Remark:         Get an operation from NdbOperation object idlelist and 
  	        ...
  	        getOperation will set the theTableId in the NdbOperation 
                object, synchronous.
*****************************************************************************/
NdbOperation *NdbTransaction::getNdbOperation(const NdbTableImpl *tab,
  ...
  if (tOp == nullptr) goto getNdbOp_error1;
...
Return Value    Return a pointer to a NdbScanOperation object if
getNdbScanOperation was successful. Return NULL : In all other case. Parameters:
aTableName : Name of the database table. Remark:         Get an operation from
NdbScanOperation idlelist and get the NdbTransaction object who was fetch by
startTransaction pointing to this  operation getOperation will set the
theTableId in the NdbOperation object.synchronous
******************************************************************************/
...
/*****************************************************************************
NdbScanOperation* getNdbIndexScanOperation(const char* anIndexName, const char*
aTableName);

Return Value    Return a pointer to a NdbIndexScanOperation object if
getNdbIndexScanOperation was successful. Return NULL : In all other case.
Parameters:     anIndexName : Name of the index to use. 	
                aTableName : Name of the database table. 	
Remark:         Get an operation from NdbIndexScanOperation idlelist and get the
NdbTransaction object who was fetch by startTransaction pointing to this
operation getOperation will set the theTableId in the NdbIndexScanOperation
object.synchronous
******************************************************************************/
NdbIndexScanOperation *NdbTransaction::getNdbIndexScanOperation(
    const char *anIndexName, const char *aTableName) {
  NdbIndexImpl *index =
...
NdbIndexScanOperation *NdbTransaction::getNdbIndexScanOperation(
  ...
  if (index) {
    ...
    if (table) return getNdbIndexScanOperation(index, table);
...
Return Value    Return a pointer to a NdbScanOperation object if
getNdbScanOperation was successful. Return NULL: In all other case. Parameters:
tableId : Id of the database table being deleted. Remark:         Get an
operation from NdbScanOperation object idlelist and get the NdbTransaction
                object who was fetch by startTransaction pointing to this
operation getOperation will set the theTableId in the NdbScanOperation object,
synchronous.
*****************************************************************************/
NdbIndexScanOperation *NdbTransaction::getNdbScanOperation(
...
void NdbTransaction::remove_list(NdbOperation *&list, NdbOperation *op) {
  NdbOperation *tmp = list;
  if (tmp == op)
    list = op->next();
  else {
    while (tmp && tmp->next() != op) tmp = tmp->next();
    if (tmp) tmp->next(op->next());
  }
  op->next(nullptr);
}

void NdbTransaction::define_scan_op(NdbIndexScanOperation *tOp) {
...
Return Value    Return a pointer to an NdbIndexOperation object if
                ...
                Return NULL : In all other case. 	
Parameters:     aTableName : Name of the database table. 	
Remark:         Get an operation from NdbIndexOperation idlelist and get the
NdbTransaction object who was fetch by startTransaction pointing to this
operation getOperation will set the theTableId in the NdbIndexOperation
object.synchronous
******************************************************************************/
NdbIndexOperation *NdbTransaction::getNdbIndexOperation(
    const char *anIndexName, const char *aTableName) {
  if (theCommitStatus == Started) {
    NdbTableImpl *table = theNdb->theDictionary->getTable(aTableName);
    NdbIndexImpl *index;
    ...
    if (table->m_frm.get_data()) {
      ...
      index =
          theNdb->theDictionary->getIndex(uniqueIndexName.c_str(), aTableName);
    } else
      index = theNdb->theDictionary->getIndex(anIndexName, aTableName);
...
Return Value    Return a pointer to a NdbIndexOperation object if
getNdbIndexOperation was successful. Return NULL: In all other case. Parameters:
tableId : Id of the database table being deleted. Remark:         Get an
operation from NdbIndexOperation object idlelist and get the NdbTransaction
                object who was fetch by startTransaction pointing to this
operation getOperation will set the theTableId in the NdbIndexOperation object,
synchronous.
*****************************************************************************/
NdbIndexOperation *NdbTransaction::getNdbIndexOperation(
    const NdbIndexImpl *anIndex, const NdbTableImpl *aTable,
    NdbOperation *aNextOp) {
...
NdbIndexOperation *NdbTransaction::getNdbIndexOperation(
  ...
  if (index) {
    ...
    if (table) return getNdbIndexOperation(index, table);
...
*******************************************************************************/
int NdbTransaction::receiveTCSEIZEREF(const NdbApiSignal *aSignal) {
  ...
  if (theStatus != Connecting) {
    ...
    DBUG_PRINT("info", ("error code %d, %s", theNdb->getNdbError().code,
...
Remark:        
******************************************************************************/
int NdbTransaction::receiveTC_COMMITCONF(const TcCommitConf *commitConf,
                                         Uint32 len) {
...
Remark:        
******************************************************************************/
int NdbTransaction::receiveTCROLLBACKCONF(const NdbApiSignal *aSignal) {
...
Remark:        
*******************************************************************************/
int NdbTransaction::receiveTCKEYCONF(const TcKeyConf *keyConf,
                                     Uint32 aDataLength) {
...
from other transactions.
  ***************************************************************************/
  if (checkState_TransId(&keyConf->transId1)) {
    ...
    if (unlikely(aDataLength <
                 TcKeyConf::StaticLength + 1 + 2 * tNoOfOperations)) {
      tGCI_lo = 0;
    }
...
*****************************************************************************/
int NdbTransaction::receiveTCKEY_FAILCONF(const TcKeyFailConf *failConf) {
  NdbOperation *tOp;
  /*
    Check that we are expecting signals from this transaction and that it 
...
**************************************************************************/
int NdbTransaction::receiveTCKEY_FAILREF(const NdbApiSignal *aSignal) {
  /*
...
*******************************************************************************/
int NdbTransaction::OpCompleteFailure() {
  Uint32 tNoComp = theNoOfOpCompleted;
...
*******************************************************************************/
int NdbTransaction::OpCompleteSuccess() {
  Uint32 tNoComp = theNoOfOpCompleted;
  Uint32 tNoSent = theNoOfOpSent;
  tNoComp++;
  theNoOfOpCompleted = tNoComp;
...
int NdbTransaction::getGCI(Uint64 *val) {
...
int NdbTransaction::getNdbErrorLine() { return theErrorLine; }
...
const NdbOperation *NdbTransaction::getNextCompletedOperation(
    const NdbOperation *current) const {
  if (current == nullptr) return theCompletedFirstOp;
  return current->theNext;
}

NdbOperation *NdbTransaction::setupRecordOp(
    NdbOperation::OperationType type, NdbOperation::LockMode lock_mode,
    NdbOperation::AbortOption default_ao, const NdbRecord *key_record,
    const char *key_row, const NdbRecord *attribute_record,
    const char *attribute_row, const unsigned char *mask,
  ...
  */
  ...
  if (opts != nullptr) {
    ...
    Uint32 result =
        NdbOperation::handleOperationOptions(type, opts, sizeOfOptions, op);
    if (result != 0) {
...
const NdbOperation *NdbTransaction::readTuple(
    const NdbRecord *key_rec, const char *key_row, const NdbRecord *result_rec,
    char *result_row, NdbOperation::LockMode lock_mode,
  ...
  NdbOperation::OperationType opType =
      (lock_mode == NdbOperation::LM_Exclusive ? NdbOperation::ReadExclusive
                                               : NdbOperation::ReadRequest);
  NdbOperation *op = setupRecordOp(
      opType, lock_mode, NdbOperation::AO_IgnoreError, key_rec, key_row,
      result_rec, result_row, result_mask, opts, sizeOfOptions);
  if (!op) return nullptr;
...
const NdbOperation *NdbTransaction::insertTuple(
    const NdbRecord *key_rec, const char *key_row, const NdbRecord *attr_rec,
  ...
  NdbOperation *op =
      setupRecordOp(NdbOperation::InsertRequest, NdbOperation::LM_Exclusive,
                    NdbOperation::AbortOnError, key_rec, key_row, attr_rec,
...
const NdbOperation *NdbTransaction::insertTuple(
    const NdbRecord *combined_rec, const char *combined_row,
    const unsigned char *mask, const NdbOperation::OperationOptions *opts,
    Uint32 sizeOfOptions) {
  return insertTuple(combined_rec, combined_row, combined_rec, combined_row,
                     mask, opts, sizeOfOptions);
}

const NdbOperation *NdbTransaction::updateTuple(
    const NdbRecord *key_rec, const char *key_row, const NdbRecord *attr_rec,
    const char *attr_row, const unsigned char *mask,
    const NdbOperation::OperationOptions *opts, Uint32 sizeOfOptions) {
  /* Check that the NdbRecord specifies the full primary key. */
  ...
  NdbOperation *op =
      setupRecordOp(NdbOperation::UpdateRequest, NdbOperation::LM_Exclusive,
                    NdbOperation::AbortOnError, key_rec, key_row, attr_rec,
                    attr_row, mask, opts, sizeOfOptions);
  if (!op) return op;
...
const NdbOperation *NdbTransaction::deleteTuple(
    const NdbRecord *key_rec, const char *key_row, const NdbRecord *result_rec,
  ...
  NdbOperation *op =
      setupRecordOp(NdbOperation::DeleteRequest, NdbOperation::LM_Exclusive,
                    NdbOperation::AbortOnError, key_rec, key_row, result_rec,
...
const NdbOperation *NdbTransaction::writeTuple(
    const NdbRecord *key_rec, const char *key_row, const NdbRecord *attr_rec,
    const char *attr_row, const unsigned char *mask,
    const NdbOperation::OperationOptions *opts, Uint32 sizeOfOptions) {
  /* Check that the NdbRecord specifies the full primary key. */
  ...
  NdbOperation *op =
      setupRecordOp(NdbOperation::WriteRequest, NdbOperation::LM_Exclusive,
                    NdbOperation::AbortOnError, key_rec, key_row, attr_rec,
                    attr_row, mask, opts, sizeOfOptions);
  if (!op) return op;
...
const NdbOperation *NdbTransaction::refreshTuple(
  ...
  NdbOperation *op =
      setupRecordOp(NdbOperation::RefreshRequest, NdbOperation::LM_Exclusive,
                    NdbOperation::AbortOnError, key_rec, key_row, key_rec,
...
NdbScanOperation *NdbTransaction::scanTable(
  ...
  /*
    ...
    objects.
  */
  NdbIndexScanOperation *op_idx = getNdbScanOperation(result_record->table);
  ...
  if (op_idx->scanTableImpl(result_record, lock_mode, result_mask, options,
                            sizeOfOptions) == 0) {
    DBUG_RETURN(op_idx);
  }
...
NdbIndexScanOperation *NdbTransaction::scanIndex(
  ...
  */
  ...
  if (op->scanIndexImpl(key_record, result_record, lock_mode, result_mask,
                        bound, options, sizeOfOptions) != 0) {
...
Uint32 NdbTransaction::getMaxPendingBlobReadBytes() const {
  /* 0 == max */
  return (maxPendingBlobReadBytes == (~Uint32(0)) ? 0
                                                  : maxPendingBlobReadBytes);
}

Uint32 NdbTransaction::getMaxPendingBlobWriteBytes() const {
  /* 0 == max */
  return (maxPendingBlobWriteBytes == (~Uint32(0)) ? 0
                                                   : maxPendingBlobWriteBytes);
}

void NdbTransaction::setMaxPendingBlobReadBytes(Uint32 bytes) {
  /* 0 == max */
  maxPendingBlobReadBytes = (bytes ? bytes : (~Uint32(0)));
}

void NdbTransaction::setMaxPendingBlobWriteBytes(Uint32 bytes) {
  DBUG_PRINT("info", ("Setting Blob max pending bytes %d", bytes));
  /* 0 == max */
...
#define CASE(x)          \
  case x:                \
    ndbout << " " << #x; \
    break
void NdbTransaction::printState() {
  ...
  CASE(DefinitionFailure);
    default:
      ndbout << (Uint32)theCompletionStatus;
  }
  ndbout << endl;
}
#undef CASE
...
int NdbTransaction::report_node_failure(Uint32 id) {
...
NdbQuery *NdbTransaction::createQuery(const NdbQueryDef *def,
...
NdbLockHandle *NdbTransaction::getLockHandle() {
  NdbLockHandle *lh;
...
const NdbOperation *NdbTransaction::unlock(const NdbLockHandle *lockHandle,
  ...
  NdbOperation *unlockOp =
      setupRecordOp(NdbOperation::UnlockRequest, NdbOperation::LM_CommittedRead,
...
int NdbTransaction::releaseLockHandle(const NdbLockHandle *lockHandle) {
