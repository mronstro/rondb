...
void BitmaskImpl::getFieldImpl(const Uint32 src[], unsigned shiftL,
...
void BitmaskImpl::setFieldImpl(Uint32 dst[], unsigned shiftL, unsigned len,
                               const Uint32 src[]) {
  /**
   *
   * abcd ef00
...
TAPTEST(Bitmask) {
  ...
  for (found = i = 0; i < MAX_BITS; i++) found += (int)b.get(i);
  OK(found == b.count());
  OK(found == 47);
  ...
  OK(d.count() == 3);
  {
    ...
    printf("toArray(): ");
    for (Uint32 i = 0; i < len; i++) printf("%u ", (Uint32)tmp[i]);
    printf("\n");
    OK(len == 3);
    OK(tmp[0] == 1);
    OK(tmp[1] == 3);
...
static Uint32 fast(Uint32 n) {
  return BitmaskImpl::count_bits(n) + BitmaskImpl::count_bits(n * n);
}

static Uint32 slow(Uint32 n) {
  double d = 1 + n;
  double l = log(d);
  double s = sqrt(d);
  double r = d * l * s;
...
inline void test_empty(Result &res, Uint32 len, unsigned iter, FUNC func) {
  Uint32 sum = 0;
  Uint64 start = NdbTick_CurrentMillisecond();
  for (Uint32 j = 0; j < iter; j++) {
    for (Uint32 k = 0; k < len; k++) sum += (*func)(k);
  }
...
inline void test_find(Result &res, const Bitmask<sz> &mask, unsigned iter,
                      FUNC func) {
  Uint32 sum = 0;
  Uint64 start = NdbTick_CurrentMillisecond();
  for (Uint32 j = 0; j < iter; j++) {
    for (Uint32 n = mask.find(0); n != mask.NotFound; n = mask.find(n + 1)) {
      sum += (*func)(n);
    }
  }
  Uint64 stop = NdbTick_CurrentMillisecond();
...
template <unsigned sz>
inline void test_find_fast(Result &res, const Bitmask<sz> &mask, unsigned iter,
                           FUNC func) {
  Uint32 sum = 0;
  Uint64 start = NdbTick_CurrentMillisecond();
  for (Uint32 j = 0; j < iter; j++) {
...
template <unsigned sz>
inline void test_find_fast_reversed(Result &res, const Bitmask<sz> &mask,
...
template <unsigned sz>
inline void test_toArray(Result &res, const Bitmask<sz> &mask, unsigned iter,
                         FUNC func) {
  Uint32 sum = 0;
  Uint64 start = NdbTick_CurrentMillisecond();
  for (Uint32 j = 0; j < iter; j++) {
    ...
    for (Uint32 n = 0; n < cnt; n++) sum += (*func)(tmp[n]);
  }
  Uint64 stop = NdbTick_CurrentMillisecond();
  res.sum += sum;
  res.elapsed += (stop - start);
...
static Uint64 sub0(Uint64 hi, Uint64 lo) {
  if (hi > lo) return hi - lo;
  return 0;
}

static Uint64 x_min(Uint64 a, Uint64 b, Uint64 c) {
  if (a < b && a < c) return a;
  if (b < a && b < c) return b;
  return c;
}

static void do_test(Uint32 len, FUNC func, const char *name, const char *dist) {
  Uint32 iter = 10000;
  if (func == slow) iter = 3000;
  ...
  res_fast_reversed.elapsed =
      sub0(res_fast_reversed.elapsed, res_empty.elapsed);
  Uint64 m =
      x_min(res_find.elapsed, res_toArray.elapsed, res_fast_reversed.elapsed);
  if (m == 0) m = 1;

  Uint64 div = iter * (10000 / len);
  printf("empty(%s,%s, %u)   : %llu ns/iter (elapsed: %llums)\n", dist, name,
         len, (1000000 * res_empty.elapsed / div / len), res_empty.elapsed);
  printf("find(%s,%s, %u)    : %llu ns/iter (%.3u%%), (sum: %u)\n", dist, name,
         len, (1000000 * res_find.elapsed / div),
         Uint32((100 * res_find.elapsed) / m), res_find.sum);
  printf("fast(%s,%s, %u)    : %llu ns/iter (%.3u%%), (sum: %u)\n", dist, name,
         len, (1000000 * res_fast.elapsed / div),
         Uint32((100 * res_fast.elapsed) / m), res_fast.sum);
  printf("toArray(%s,%s, %u) : %llu ns/iter (%.3u%%), (sum: %u)\n", dist, name,
         len, (1000000 * res_toArray.elapsed / div),
         Uint32((100 * res_toArray.elapsed) / m), res_toArray.sum);
  printf("reversed(%s,%s, %u)    : %llu ns/iter (%.3u%%), (sum: %u)\n", dist,
         name, len, (1000000 * res_fast_reversed.elapsed / div),
         Uint32((100 * res_fast_reversed.elapsed) / m), res_fast_reversed.sum);
  printf("toArray(%s,%s, %u) : %llu ns/iter (%.3u%%), (sum: %u)\n", dist, name,
         len, (1000000 * res_toArray.elapsed / div),
         Uint32((100 * res_toArray.elapsed) / m), res_toArray.sum);
  printf("\n");
