...
Uint32 packedSignalLength(Uint32 signalType)
{
  switch(signalType)
  {
  case ZCOMMIT: return 5;
  case ZCOMPLETE: return 3;
  case ZCOMMITTED: return 3;
  case ZCOMPLETED: return 3;
  case ZLQHKEYCONF: return LqhKeyConf::SignalLength;
  case ZREMOVE_MARKER: return 3;
  case ZFIRE_TRIG_REQ: return FireTrigReq::SignalLength;
  case ZFIRE_TRIG_CONF: return FireTrigConf::SignalLength;
  default: return RNIL;
  }
}
...
const char* packedSignalName(Uint32 signalType, bool failApi)
{
  switch(signalType)
  {
  case ZCOMMIT: return "COMMIT";
  case ZCOMPLETE: return "COMPLETE";
  case ZCOMMITTED: return "COMMITTED";
  case ZCOMPLETED: return "COMPLETED";
  case ZLQHKEYCONF: return "LQHKEYCONF";
  case ZREMOVE_MARKER: return failApi ? "REMOVE_MARKER_FAIL_API" : "REMOVE_MARKER";
  case ZFIRE_TRIG_REQ: return "FIRE_TRIG_REQ";
  case ZFIRE_TRIG_CONF: return "FIRE_TRIG_CONF";
  default: return 0;
  }
}
...
printPACKED_SIGNAL(FILE * output, const Uint32 * theData, Uint32 len, Uint16 receiverBlockNo){
  ...
  for(Uint32 i = 0; i < len;)
  {
    ...
    switch(signalType)
    {
    case ZCOMMIT: case ZCOMPLETE: case ZCOMMITTED: case ZCOMPLETED:
    case ZLQHKEYCONF: case ZREMOVE_MARKER: case ZFIRE_TRIG_REQ:
  ...
  for(int i = -1; i < numberOfSignals; i++) {
     ...
     * 0<=idx<numberOfSignals. We handle the special case idx==-1 inside the
     * loop to make it easier to reverse the order.
     */
    ...
    if(0 <= idx) // Print a packed signal
    {
      ...
      case ZREMOVE_MARKER:
        ...
        fprintf(output,"\n");
        break;
      case ZFIRE_TRIG_REQ: case ZFIRE_TRIG_CONF:
        break;
      default:
        fprintf(output, "  ----------- Packed signal %d -----------\n", idx);
