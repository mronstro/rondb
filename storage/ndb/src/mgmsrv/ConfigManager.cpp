...
static bool alone_on_host(Config *conf, Uint32 own_type, Uint32 own_nodeid) {
  ConfigIter iter(conf, CFG_SECTION_NODE);
  for (iter.first(); iter.valid(); iter.next()) {
    Uint32 type;
    if (iter.get(CFG_TYPE_OF_SECTION, &type) || type != own_type) {
      continue;
    }

    Uint32 nodeid;
    if (iter.get(CFG_NODE_ID, &nodeid) || nodeid == own_nodeid) {
      continue;
    }
    ...
    if (Ndb_getAddr(&addr, hostname) == -1) {
      g_eventLogger->debug("Failed resolve %s, node %d", hostname, nodeid);
      continue;
    }
...
NodeId ConfigManager::find_nodeid_from_configdir(void) {
  BaseString config_name;
  NdbDir::Iterator iter;

  if (!m_configdir || iter.open(m_configdir) != 0) {
    return 0;
  }
  ...
  while ((name = iter.next_file()) != NULL) {
    if (sscanf(name, "ndb_%u_config.bin.%u%c", &nodeid, &version, &extra) ==
        2) {
  ...
  config_name.assfmt("%s%sndb_%u_config.bin.%u", m_configdir, DIR_SEPARATOR,
                     found_nodeid, max_version);
  ...
  if (!m_config_retriever.verifyConfig(conf->m_configuration, found_nodeid) ||
...
static NodeId find_own_nodeid(Config *conf) {
  ...
  for (iter.first(); iter.valid(); iter.next()) {
    Uint32 type;
    if (iter.get(CFG_TYPE_OF_SECTION, &type) || type != NDB_MGM_NODE_TYPE_MGM) {
      continue;
    }
...
NodeId ConfigManager::find_nodeid_from_config(void) {
...
bool ConfigManager::init_nodeid(void) {
  ...
  if (nodeid) {
    ...
    g_eventLogger->debug("Got nodeid: %d from searching in configdir", nodeid);
    m_node_id = nodeid;
    DBUG_RETURN(true);
  }
...
static void reset_dynamic_ports_in_config(const Config *config) {
...
bool ConfigManager::init(void) {
  ...
  if (!init_nodeid()) DBUG_RETURN(false);
  ...
  if (failed_config_change_exists()) DBUG_RETURN(false);
  ...
  if (saved_config_exists(config_bin_name)) {
    ...
    Config *conf = NULL;
    if (!(conf = load_saved_config(config_bin_name))) DBUG_RETURN(false);

    if (!config_ok(conf)) DBUG_RETURN(false);
    ...
    if (m_opts.reload &&  // --reload
      ...
      if (new_conf == NULL) DBUG_RETURN(false);
    ...
    if (m_opts.mycnf || m_opts.config_filename) {
      Config *conf = load_config();
      if (conf == NULL) DBUG_RETURN(false);
...
bool ConfigManager::prepareConfigChange(const Config *config) {
  ...
  assert(m_node_id);
  m_config_name.assfmt("%s%sndb_%u_config.bin.%u", m_configdir, DIR_SEPARATOR,
                       m_node_id, generation);
...
ConfigManager::commitConfigChange(void)
{
  ...
  if (rename(prep_config_name.c_str(), m_config_name.c_str())) {
    g_eventLogger->error(
        ...
        "errno: %d",
        prep_config_name.c_str(), m_config_name.c_str(), errno);
...
static void check_no_dynamic_ports_in_config(const Config *config) {
  bool ok = true;
  ConfigIter iter(config, CFG_SECTION_CONNECTION);
...
void ConfigManager::set_config(Config *new_config) {
...
bool ConfigManager::config_ok(const Config *conf) {
  ...
  if (!(m_started.get(m_node_id) || m_opts.print_full_config)) {
    ...
    for (it.first(); it.valid(); it.next()) {
      ...
      if (num_mgm_nodes > 1) {
        g_eventLogger->info(
            "Cluster configuration has multiple "
...
void ConfigManager::sendConfigChangeImplRef(
    SignalSender &ss, NodeId nodeId, ConfigChangeRef::ErrorCode error) const {
  SimpleSignal ssig;
  ...
  ss.sendSignal(nodeId, ssig, MGM_CONFIG_MAN, GSN_CONFIG_CHANGE_IMPL_REF,
                ConfigChangeImplRef::SignalLength);
}

void ConfigManager::execCONFIG_CHANGE_IMPL_REQ(SignalSender &ss,
                                               SimpleSignal *sig) {
  ...
  if (!m_defragger.defragment(sig)) return;  // More fragments to come
  ...
  switch (req->requestType) {
    case ConfigChangeImplReq::Prepare: {
      ...
      bool ret = v2 ? cf.unpack_v2(sig->ptr[0].p, req->length)
      ...
      if (m_config->illegal_change(&new_config)) {
        sendConfigChangeImplRef(ss, nodeId,
                                ConfigChangeRef::IllegalConfigChange);
        return;
      }
      ...
      if (req->initial) {
        ...
        if (nodeId != refToNode(ss.getOwnRef())) {
          ...
          unsigned exclude[] = {CFG_SECTION_SYSTEM, 0};
          if (!new_config_copy.equal(m_config_change.m_initial_config,
                                     exclude)) {
            BaseString buf;
            g_eventLogger->warning(
                ...
                new_config_copy.diff2str(m_config_change.m_initial_config,
                                         buf));
            sendConfigChangeImplRef(ss, nodeId,
        ...
        if (new_generation == 0 || new_generation != curr_generation) {
          BaseString buf;
          g_eventLogger->warning(
          ...
          sendConfigChangeImplRef(ss, nodeId,
                                  ConfigChangeRef::InvalidGeneration);
          return;
        }
        new_generation++;
        ...
        if (strcmp(new_name, curr_name)) {
          ...
          g_eventLogger->warning(
              ...
              "name: '%s'. Our name: '%s'\n"
              "This is the actual diff:\n%s",
              new_name, curr_name, new_config.diff2str(m_config, buf));
          sendConfigChangeImplRef(ss, nodeId,
                                  ConfigChangeRef::InvalidConfigName);
          return;
        }
      }
  ...
  g_eventLogger->debug("Sending CONFIG_CHANGE_IMPL_CONF to node: %d", nodeId);

  ss.sendSignal(nodeId, ssig, MGM_CONFIG_MAN, GSN_CONFIG_CHANGE_IMPL_CONF,
...
void ConfigManager::execCONFIG_CHANGE_IMPL_REF(SignalSender &ss,
                                               SimpleSignal *sig) {
  NodeId nodeId = refToNode(sig->header.theSendersBlockRef);
  ...
  switch (m_config_change.m_state) {
    ...
    case ConfigChangeState::PREPARING: {
      ...
      if (!m_waiting_for.isclear()) return;
...
void ConfigManager::execCONFIG_CHANGE_IMPL_CONF(SignalSender &ss,
                                                SimpleSignal *sig) {
  NodeId nodeId = refToNode(sig->header.theSendersBlockRef);
  ...
  switch (m_config_change.m_state) {
    case ConfigChangeState::PREPARING: {
      ...
      m_waiting_for = ss.broadcastSignal(
          m_config_change.m_contacted_nodes, ssig, MGM_CONFIG_MAN,
          GSN_CONFIG_CHANGE_IMPL_REQ, ConfigChangeImplReq::SignalLength);
...
void ConfigManager::sendConfigChangeRef(
    SignalSender &ss, BlockReference to,
    ConfigChangeRef::ErrorCode error) const {
  NodeId nodeId = refToNode(to);
  SimpleSignal ssig;
  ConfigChangeRef *const ref = CAST_PTR(ConfigChangeRef, ssig.getDataPtrSend());
  ref->errorCode = error;

  g_eventLogger->debug("Send CONFIG_CHANGE_REF to node: %d, error: %d", nodeId,
                       error);

  ss.sendSignal(nodeId, ssig, refToBlock(to), GSN_CONFIG_CHANGE_REF,
                ConfigChangeRef::SignalLength);
}

void ConfigManager::sendConfigChangeConf(SignalSender &ss,
                                         BlockReference to) const {
  NodeId nodeId = refToNode(to);
  SimpleSignal ssig;

  g_eventLogger->debug("Send CONFIG_CHANGE_CONF to node: %d", nodeId);

  ss.sendSignal(nodeId, ssig, refToBlock(to), GSN_CONFIG_CHANGE_CONF,
                ConfigChangeConf::SignalLength);
}

void ConfigManager::startConfigChange(SignalSender &ss, Uint32 ref) {
  if (m_config_state == CS_INITIAL) {
...
void ConfigManager::startAbortConfigChange(SignalSender &ss) {
  ...
  m_waiting_for = ss.broadcastSignal(m_config_change.m_contacted_nodes, ssig,
                                     MGM_CONFIG_MAN, GSN_CONFIG_CHANGE_IMPL_REQ,
  ...
  if (m_waiting_for.isclear()) {
    ...
    ss.sendSignal(ss.getOwnNodeId(), ssig, MGM_CONFIG_MAN,
...
int ConfigManager::sendConfigChangeImplReq(SignalSender &ss,
                                           const Config *conf) {
  require(m_waiting_for.isclear());
  require(m_config_change.m_client_ref != RNIL);
...
void ConfigManager::execCONFIG_CHANGE_REQ(SignalSender &ss, SimpleSignal *sig) {
...
static Uint32 config_check_checksum(const Config *config, bool v2) {
  Config copy(config);
...
void ConfigManager::execCONFIG_CHECK_REQ(SignalSender &ss, SimpleSignal *sig) {
  Guard g(m_config_mutex);
  BlockReference from = sig->header.theSendersBlockRef;
  ...
  if (ERROR_INSERTED(100) && nodeId != ss.getOwnNodeId()) {
    g_eventLogger->debug(
        "execCONFIG_CHECK_REQ() ERROR_INSERTED(100) => exit()");
    exit(0);
  }
  ...
  if (m_prepared_config || m_config_change.m_new_config) {
    g_eventLogger->debug(
        ...
        nodeId);
    sendConfigCheckRef(ss, from, ConfigCheckRef::WrongState, generation,
                       other_generation, m_config_state, CS_UNINITIALIZED);
    return;
  }
  /**
  ...
  switch (m_config_state) {
    ...
    case CS_INITIAL:
      if (other_state != CS_INITIAL) {
        g_eventLogger->warning(
            "Refusing CONFIG_CHECK_REQ from %u, "
            "  it's not CS_INITIAL (I am). "
            " Waiting for my check",
            nodeId);
        sendConfigCheckRef(ss, from, ConfigCheckRef::WrongState, generation,
                           other_generation, m_config_state, other_state);
        return;
      }
      ...
      if (other_generation != generation) {
        ...
        sendConfigCheckRef(ss, from, ConfigCheckRef::WrongGeneration,
                           generation, other_generation, m_config_state,
                           other_state);
        return;
      }
      ...
      if (other_checksum != checksum) {
        g_eventLogger->warning(
            ...
            other_checksum, checksum);
        sendConfigCheckRef(ss, from, ConfigCheckRef::WrongChecksum, generation,
                           other_generation, m_config_state, other_state);
        return;
      }
      break;
...
void ConfigManager::sendConfigCheckReq(SignalSender &ss, NodeBitmask to) {
  SimpleSignal ssig;
  ConfigCheckReq *const req = CAST_PTR(ConfigCheckReq, ssig.getDataPtrSend());
  ...
  while (nodeId != to.NotFound) {
    ...
    m_waiting_for.set(nodeId);
    ss.sendSignal(nodeId, ssig, MGM_CONFIG_MAN, GSN_CONFIG_CHECK_REQ,
                  ConfigCheckReq::SignalLength);
    nodeId = to.find(nodeId + 1);
  }
}

static bool send_config_in_check_ref(Uint32 x) {
  if (x >= NDB_MAKE_VERSION(7, 0, 8)) return true;
  return false;
}

void ConfigManager::sendConfigCheckRef(SignalSender &ss, BlockReference to,
  ...
  NodeId nodeId = refToNode(to);
  SimpleSignal ssig;
  ConfigCheckRef *const ref = CAST_PTR(ConfigCheckRef, ssig.getDataPtrSend());
  ref->error = error;
  ...
  if (!send_config_in_check_ref(ss.getNodeInfo(nodeId).m_info.m_version)) {
    result = ss.sendSignal(nodeId, ssig, MGM_CONFIG_MAN, GSN_CONFIG_CHECK_REF,
                           ConfigCheckRef::SignalLength);
  } else {
    const Uint32 version_receiving = ss.getNodeInfo(nodeId).m_info.m_version;
    bool v2 = ndb_config_version_v2(version_receiving);
    UtilBuffer buf;
...
void ConfigManager::sendConfigCheckConf(SignalSender &ss,
                                        BlockReference to) const {
  NodeId nodeId = refToNode(to);
  SimpleSignal ssig;
  ...
  ss.sendSignal(nodeId, ssig, MGM_CONFIG_MAN, GSN_CONFIG_CHECK_CONF,
                ConfigCheckConf::SignalLength);
}

void ConfigManager::execCONFIG_CHECK_CONF(SignalSender &ss, SimpleSignal *sig) {
  BlockReference from = sig->header.theSendersBlockRef;
  NodeId nodeId = refToNode(from);
...
void ConfigManager::execCONFIG_CHECK_REF(SignalSender &ss, SimpleSignal *sig) {
  BlockReference from = sig->header.theSendersBlockRef;
  NodeId nodeId = refToNode(from);
  assert(m_waiting_for.get(nodeId));
  ...
  g_eventLogger->debug(
      ...
      nodeId, ref->error, ConfigCheckRef::errorMessage(ref->error),
      ref->generation, ref->expected_generation, ref->state,
      ref->expected_state, m_config_state);
  ...
  switch (m_config_state) {
    ...
    case CS_INITIAL:
      if (ref->expected_state == CS_CONFIRMED) {
        ...
        bool v2 = ndb_config_version_v2(version_sending);
        ConfigValuesFactory cf;
        bool ret = v2 ? cf.unpack_v2(sig->ptr[0].p, ref->length)
        ...
        if (!other_config.equal(m_config, exclude)) {
          ...
          g_eventLogger->error(
              ...
              nodeId, other_config.diff2str(m_config, buf, exclude));
          exit(1);
        }
  ...
  if (ref->error == ConfigCheckRef::WrongChecksum && m_node_id < nodeId) {
    g_eventLogger->warning(
...
void ConfigManager::set_facade(TransporterFacade *f) {
  m_facade = f;
...
bool ConfigManager::ConfigChange::config_loaded(Config *config) {
  if (m_loaded_config != 0) return false;
  m_loaded_config = config;
  return true;
}

Config *ConfigManager::prepareLoadedConfig(Config *new_conf) {
      ...
      switch (m_config_state) {
        ...
        case CS_INITIAL:
          ...
          */
          ...
          if (print_state) ndbout_c("==INITIAL==");

          if (m_config_change
                  .m_initial_config &&       // Updated config.ini was found
    ...
    switch (sig->readSignalNumber()) {
      ...
      default:
        ...
        g_eventLogger->error(
            ...
            "%i from (%d, 0x%x)",
            sig->readSignalNumber(), refToNode(sig->header.theSendersBlockRef),
...
Config *ConfigManager::load_config(const char *config_filename, bool mycnf,
                                   BaseString &msg,
  ...
  if (mycnf && (new_conf = load_init_mycnf(cluster_config_suffix)) == nullptr) {
    ...
    msg.assfmt("Could not load configuration from '%s'", config_filename);
    return nullptr;
  }
...
Config *ConfigManager::load_config(void) const {
  BaseString msg;
  Config *new_conf = load_config(m_opts.config_filename, m_opts.mycnf, msg,
...
Config *ConfigManager::fetch_config(void) {
  DBUG_ENTER("ConfigManager::fetch_config");

  m_config_retriever.init_mgm_tls(m_opts.tls_search_path, Node::Type::MGMD,
                                  m_opts.mgm_tls);
  ...
  while (true) {
    ...
    if (!m_config_retriever.is_connected()) {
      int ret = m_config_retriever.do_connect(30 /* retry */, 1 /* delay */,
                                              0 /* verbose */);
      if (ret == 0) {
        // connection success
...
static bool delete_file(const char *file_name) {
  ...
  if (DeleteFile(file_name) == 0) {
    g_eventLogger->error("Failed to delete file '%s', error: %d", file_name,
                         GetLastError());
    return false;
  }
#else
...
bool ConfigManager::delete_saved_configs(void) const {
  NdbDir::Iterator iter;
  ...
  if (iter.open(m_configdir) != 0) return false;
...
bool ConfigManager::saved_config_exists(BaseString &config_name) const {
  NdbDir::Iterator iter;

  if (!m_configdir || iter.open(m_configdir) != 0) return 0;
  ...
  if (max_version == 0) return false;

  config_name.assfmt("%s%sndb_%u_config.bin.%u", m_configdir, DIR_SEPARATOR,
                     m_node_id, max_version);
  return true;
}

bool ConfigManager::failed_config_change_exists() const {
  NdbDir::Iterator iter;
  ...
  while ((name = iter.next_file()) != NULL) {
    ...
    if (sscanf(name, "ndb_%u_config.bin.%u.tm%c", &nodeid, &version, &tmp) ==
            3 &&
        tmp == 'p') {
      if (nodeid != m_node_id) continue;
...
Config *ConfigManager::load_saved_config(const BaseString &config_name) {
  ...
  if (!conf) g_eventLogger->error("Failed to load config, out of memory");
  return conf;
}

bool ConfigManager::get_packed_config(ndb_mgm_node_type nodetype,
                                      BaseString *buf64, BaseString &error,
                                      bool v2, Uint32 node_id) {
  Guard g(m_config_mutex);
...
static bool check_dynamic_port_configured(const Config *config, int node1,
                                          int node2, BaseString &msg) {
  ConfigIter iter(config, CFG_SECTION_CONNECTION);
  ...
  if (!iter.valid()) {
    msg.assfmt("Unable to find connection between nodes %d -> %d", node1,
               node2);
    return false;
  }
...
bool ConfigManager::set_dynamic_port(int node1, int node2, int value,
                                     BaseString &msg) {
...
bool ConfigManager::set_dynamic_ports(int node, MgmtSrvr::DynPortSpec ports[],
  ...
  for (unsigned i = 0; i < num_ports; i++) {
    ...
    if (!check_dynamic_port_configured(m_config, node, node2, msg)) {
      return false;
    }
  }
...
bool ConfigManager::get_dynamic_port(int node1, int node2, int *value,
                                     BaseString &msg) const {
  Guard g(m_config_mutex);
  if (!check_dynamic_port_configured(m_config, node1, node2, msg)) return false;
...
bool ConfigManager::DynamicPorts::set(int node1, int node2, int port) {
  ...
  if (!m_ports.insert(NodePair(node1, node2), port, true)) return false;
...
bool ConfigManager::DynamicPorts::set_in_config(Config *config) {
  bool result = true;
  ConfigIter iter(config, CFG_SECTION_CONNECTION);

  for (; iter.valid(); iter.next()) {
    Uint32 port = 0;
    if (iter.get(CFG_CONNECTION_SERVER_PORT, &port) != 0 || port != 0)
    ...
    ConfigValues::Iterator i2(config->m_configuration->m_config_values,
                              iter.m_config);
    if (i2.set(CFG_CONNECTION_SERVER_PORT, port) == false) result = false;
  }
  return result;
}
