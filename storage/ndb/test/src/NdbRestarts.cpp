...
#define F_ARGS                                 \
  NDBT_Context *ctx, NdbRestarter &_restarter, \
      const NdbRestarts::NdbRestart *_restart, int
...
const NdbRestarts::NdbRestart NdbRestarts::m_restarts[] = {
     ...
     */
    NdbRestart("RestartRandomNode", NODE_RESTART, restartRandomNodeGraceful, 2,
               1),
    /**
     * Restart a randomly selected node
     * with immediate(abort) shutdown
     */
    NdbRestart("RestartRandomNodeAbort", NODE_RESTART, restartRandomNodeAbort,
               2, 1),
    /**
     * Restart a randomly selected node
     * with  error insert
     *
     */
    NdbRestart("RestartRandomNodeError", NODE_RESTART, restartRandomNodeError,
               2, 1),
    /**
     * Restart the master node
     * with  error insert
     */
    NdbRestart("RestartMasterNodeError", NODE_RESTART, restartMasterNodeError,
               2, 1),
    /**
     * Restart a randomly selected node without file system
     *
     */
    NdbRestart("RestartRandomNodeInitial", NODE_RESTART,
               restartRandomNodeInitial, 2, 1),
    /**
     * Restart a randomly selected node and then
     * crash it while restarting
     *
     */
    NdbRestart("RestartNFDuringNR", NODE_RESTART, restartNFDuringNR, 2, 1),
     ...
     *
     */
    NdbRestart("StopOnError", NODE_RESTART, stopOnError, 1, 1),
     ...
     */
    NdbRestart("TwoNodeFailure", MULTIPLE_NODE_RESTART, twoNodeFailure, 3, 1),
    /**
     * 2 nodes restart, select master nodes and restart with
     * a small random delay between restarts
     */
    NdbRestart("TwoMasterNodeFailure", MULTIPLE_NODE_RESTART,
               twoMasterNodeFailure, 3, 2),

    NdbRestart("FiftyPercentFail", MULTIPLE_NODE_RESTART, fiftyPercentFail, 2,
               1),
    ...
    NdbRestart("RestartAllNodes", SYSTEM_RESTART, restartAllNodesGracfeul, 1,
               1),
    /**
     * Restart all nodes immediately without
     * graful shutdown
     */
    NdbRestart("RestartAllNodesAbort", SYSTEM_RESTART, restartAllNodesAbort, 1,
               1),
    /**
     * Restart all nodes with error insert 9999
     * TODO! We can later add more errors like 9998, 9997 etc.
     */
    NdbRestart("RestartAllNodesError9999", SYSTEM_RESTART,
               restartAllNodesError9999, 1, 1),
    /**
     ...
     */
    NdbRestart("FiftyPercentStopAndWait", SYSTEM_RESTART,
               fiftyPercentStopAndWait, 2, 1),
    /**
     * Restart a master node during LCP with error inserts.
     */
    NdbRestart("RestartNodeDuringLCP", NODE_RESTART, restartNodeDuringLCP, 2,
               1),
};
...
NdbRestarts::NdbRestart::NdbRestart(const char *_name, NdbRestartType _type,
                                    restartFunc *_func, int _requiredNodes,
...
int NdbRestarts::getNumRestarts() { return m_NoOfRestarts; }

const NdbRestarts::NdbRestart *NdbRestarts::getRestart(int _num) {
  if (_num >= m_NoOfRestarts) return NULL;
...
int NdbRestarts::executeRestart(NDBT_Context *ctx,
  ...
  // for this test
  {
    ...
    if (config_node_groups < min_groups) {
      g_err << "This test requires " << min_groups
            << " node groups, "
               "but cluster has only "
            << config_node_groups << "." << endl;
      return NDBT_OK;
    }
  }
...
int NdbRestarts::executeRestart(NDBT_Context *ctx, int _num,
                                unsigned int _timeout, int safety) {
...
int NdbRestarts::executeRestart(NDBT_Context *ctx, const char *_name,
                                unsigned int _timeout, int safety) {
  ...
  for (int i = 0; i < m_NoOfRestarts; i++) {
    if (m_restarts[i].m_type == _type)
      ndbout << " " << m_restarts[i].m_name << ", min "
             << m_restarts[i].m_numRequiredNodes << " nodes" << endl;
  }
}
...
NdbRestarts::NdbErrorInsert::NdbErrorInsert(const char *_name, int _errorNo) {
  m_name = _name;
  m_errorNo = _errorNo;
}

int NdbRestarts::getNumErrorInserts() { return m_NoOfErrors; }

const NdbRestarts::NdbErrorInsert *NdbRestarts::getError(int _num) {
  if (_num >= m_NoOfErrors) return NULL;
...
int twoNodeFailure(F_ARGS) {
  ...
  CHECK(_restarter.dumpStateOneNode(n[0], val2, 2) == 0, "Failed to dump");
  CHECK(_restarter.dumpStateOneNode(n[1], val2, 2) == 0, "Failed to dump");
  ...
  g_info << "Waiting for " << ms << "(" << max << ") ms " << endl;
  ...
  CHECK(_restarter.waitNodesNoStart(n, 2) == 0, "Failed to wait nostart");
...
int get50PercentOfNodes(NdbRestarter &restarter, int *_nodes) {
...
int fiftyPercentFail(F_ARGS) {
  ...
  CHECK(_restarter.waitNodesNoStart(nodes, numNodes) == 0, "waitNodesNoStart");

  // Order all nodes to start
  ndbout << "Starting all nodes" << endl;
  CHECK(_restarter.startAll() == 0, "Could not start all nodes");
...
int restartAllNodesGracfeul(F_ARGS) {
  ...
  // Restart graceful
  CHECK(_restarter.restartAll() == 0, "Could not restart all nodes");
...
int restartAllNodesError9999(F_ARGS) {
  ...
  CHECK(_restarter.waitClusterNoStart() == 0, "Failed to wait not started");
...
int fiftyPercentStopAndWait(F_ARGS) {
  ...
  g_info << "Waiting for " << seconds << "(" << max << ") secs " << endl;
...
int NFDuringNR_codes[] = {7121, 5027, 7172, 6000, 6001, 7171, 7130, 7133, 7138,
                          7154, 7144, 5026, 7139, 7132, 5045,

                          7195, 7196, 7197, 7198, 7199,

                          // LCP
                          8000, 8001, 5010, 7022, 7024, 7016, 7017, 5002};
...
int restartNFDuringNR(F_ARGS safety) {
  ...
  if (_restarter.getNumNodeGroups() < 2) return NDBT_OK;

  for (i = 0; i < sz && !ctx->isTestStopped() && !ctx->closeToTimeout(safety);
       i++) {
    ...
    g_info << _restart->m_name << " restarting node = " << nodeId
           << " error code = " << error << " crash node = " << crashNodeId
           << endl;
...
int NRDuringLCP_Master_codes[] = {
    ...
    7019,  // Insert system error in master when local checkpoint is in the
           // state clcpStatus = IDLE before sending
           // CONTINUEB(ZCHECK_TC_COUNTER).
...
int NRDuringLCP_NonMaster_codes[] = {
...
int restartNodeDuringLCP(F_ARGS safety) {
  ...
  int sz =
      sizeof(NRDuringLCP_Master_codes) / sizeof(NRDuringLCP_Master_codes[0]);
  for (i = 0; i < sz && !ctx->closeToTimeout(safety); i++) {
    ...
    CHECK(_restarter.startNodes(&masterNodeId, 1) == 0, "failed to start node");

    CHECK(_restarter.waitClusterStarted(300) == 0, "waitClusterStarted failed");
