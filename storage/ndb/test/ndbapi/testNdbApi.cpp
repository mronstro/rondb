    ...
    do {
      ...
      switch (type) {
        ...
        case 1: {
          ...
          pCon = pNdb->startTransaction(pTab, key.c_str(), key.length());
        } break;
        default:
          abort();
      }
...
int runTestMaxOperations(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  int upper_max_ops = -1;
  bool endTest;
  while (upper_max_ops == -1 || (upper_max_ops - lower_max_ops) >= 1000) {
    int errors = 0;
    ...
    for (int i = 1; !endTest && i <= next_max_ops; i++) {
      ...
      require(execResult == 0 || execResult == err.code);
      ...
      switch (execResult) {
        ...
        default:
          result = NDBT_FAILED;
          //  261: //Increased beyond MaxDMLOperationsPerTransaction or
          //  MaxNoOfConcurrentOperations
          ndbout_c("Got unexpected error %u %s for non DML transaction",
                   err.code, err.message);
          [[fallthrough]];
        case 233:   // Out of operation records in transaction coordinator -
                    // SharedGlobalMemory
        case 234:   // Out of operation records in transaction coordinator -
                    // MaxNoOfConcurrentOperations
        case 1217:  // Out of operation records in local data manager (increase
                    // MaxNoOfLocalOperations)

          /* Ok, check that error is temporary */
          if (err.status != NdbError::TemporaryError) {
            ndbout_c("Error : non temporary error %u %s returned", err.code,
                     err.message);
            result = NDBT_FAILED;
          }
          // OK - end test
    ...
    ndbout << next_max_ops << " operations used"
           << (endTest ? " failed" : " succeeded") << endl;
...
int runTestDeleteNdb(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (l < loops && result == NDBT_OK) {
    ...
    for (unsigned j = 0; j < ndbVector.size(); j++) delete ndbVector[j];
    ndbVector.clear();
    l++;
  }
...
int runTestIgnoreError(NDBT_Context *ctx, NDBT_Step *step) {
...
static Uint32 do_cnt(Ndb_cluster_connection *con) {
  Uint32 cnt = 0;
  const Ndb *p = 0;
...
int runNdbClusterConnectionDelete_connection_owner(NDBT_Context *ctx,
  ...
  char constr[256];
  if (!ctx->m_cluster_connection.get_connectstring(constr, sizeof(constr))) {
...
int runNdbClusterConnectionDelete_connection_user(NDBT_Context *ctx,
                                                  NDBT_Step *step) {
   ...
   public:
    void release() {
      while (m_transactions.size()) {
        ...
        g_info << "Deleting Ndb object " << ndb << "and transaction " << trans
               << endl;
    ...
    void push_back(NdbTransaction *trans) { m_transactions.push_back(trans); }
  } active_transactions;
...
static void testExecuteAsynchCallback(int res, NdbTransaction *con,
                                      void *data_ptr) {
...
int simpleReadAbortOnError(NDBT_Context *ctx, NDBT_Step *step) {
    ...
    for (int record = 0; record < numRecords; record++) {
      ...
      for (int col = 0; col < pTab->getNoOfColumns(); col++) {
        ...
        if (pTab->getColumn(col)->getNullable()) {
          NdbDictionary::setNull(tabRec, keyRowBuf, col, isNull);
        }
      }
      ...
      while (memcmp(keyRowBuf, attrRowBuf, sizeOfTabRec) == 0) {
        ...
        for (int col = 0; col < pTab->getNoOfColumns(); col++) {
          if (pTab->getColumn(col)->getPrimaryKey()) {
            ...
            bool isNull = (calc.calcValue(badRecord, col, updates, valPtr, len,
                                          &real_len) == NULL);
            CHECK(!isNull);
          }
        }
      ...
      if (optype == 0) {
        ...
        op = trans->writeTuple(tabRec, keyRowBuf, tabRec, attrRowBuf);
      }
      CHECK(op != 0);
      ...
      op = trans->readTuple(tabRec, keyRowBuf, tabRec, attrRowBuf);
      CHECK(op != 0);
      CHECK(trans->execute(Commit) == 0);
...
int testNdbRecordPKUpdate(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (int record = 0; record < numRecords; record++) {
    ...
    for (int col = 0; col < pTab->getNoOfColumns(); col++) {
      if (calc.isIdCol(col)) {
        char *valPtr = NdbDictionary::getValuePtr(tabRec, badKeyRowBuf, col);
    ...
    trans = pNdb->startTransaction();
    op = trans->updateTuple(tabRec, rowBuf, tabRec, rowBuf);
    CHECK(op != 0);
    CHECK(trans->execute(Commit) == 0);
    CHECK(trans->getNdbError().code == 0);
    trans->close();
    ...
    NdbScanOperation *scanOp =
        trans->scanTable(tabRec, NdbOperation::LM_Exclusive);
    CHECK(scanOp != 0);
    ...
    scanOp = trans->scanTable(tabRec, NdbOperation::LM_Exclusive);
    CHECK(scanOp != 0);
    ...
    /* Now delete the tuple */
    trans = pNdb->startTransaction();
    op = trans->deleteTuple(tabRec, rowBuf, tabRec);
    CHECK(op != 0);
    CHECK(trans->execute(Commit) == 0);
...
int testNdbRecordCICharPKUpdate(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  char readBuf[rowLen];
  char *ucPkPtr = NdbDictionary::getValuePtr(tabRec, ucRowBuf, 0);
  Uint32 *ucDataPtr = (Uint32 *)NdbDictionary::getValuePtr(tabRec, ucRowBuf, 1);
  char *lcPkPtr = NdbDictionary::getValuePtr(tabRec, lcRowBuf, 0);
  Uint32 *lcDataPtr = (Uint32 *)NdbDictionary::getValuePtr(tabRec, lcRowBuf, 1);

  char *readPkPtr = NdbDictionary::getValuePtr(tabRec, readBuf, 0);
  Uint32 *readDataPtr =
      (Uint32 *)NdbDictionary::getValuePtr(tabRec, readBuf, 1);
  ...
  for (int record = 0; record < numRecords; record++) {
    ...
    mask[0] = 1;
    op = trans->updateTuple(tabRec, lcRowBuf, tabRec, lcRowBuf, mask);
    CHECK(op != 0);
    CHECK(trans->execute(Commit) == 0);
    CHECK(trans->getNdbError().code == 0);
    trans->close();
...
int testPKUpdateWithSetValue(NDBT_Context *ctx, NDBT_Step *step) {
  /* Bug#35106292:
  ...
  pNdb->getDictionary()->dropTable(tab.getName());
  CHECKE(pNdb->getDictionary()->createTable(tab) == 0,
         (*pNdb->getDictionary()));
  pTab = pNdb->getDictionary()->getTable(tab.getName());
  ...
  for (int i = 0; i < 2; i++) {
    ...
    *pk2Ptr = 1;
    ...
    CHECK(pTrans = pNdb->startTransaction());
    if (i == 0) {
      CHECKE(pTrans->updateTuple(tabRec, rowBuf, tabRec, NULL, &mask, &opts,
                                 sizeof(opts)),
             (*pTrans));
    } else {
      CHECKE(pTrans->writeTuple(tabRec, rowBuf, tabRec, NULL, &mask, &opts,
                                sizeof(opts)),
             (*pTrans));
    }
    CHECKE(pTrans->execute(NoCommit) == 0, (*pTrans));
...
int testNdbRecordRowLength(NDBT_Context *ctx, NDBT_Step *step) {
...
int runBug44015(NDBT_Context *ctx, NDBT_Step *step) {
...
int runScanReadUntilStopped(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (ctx->isTestStopped() == false) {
    g_info << i << ": ";
    if (hugoTrans.scanReadRecords(GETNDB(step), 0, 0, 0, lm, scan_flags) != 0) {
      return NDBT_FAILED;
    }
    i++;
  }
...
int runBug44065_org(NDBT_Context *ctx, NDBT_Step *step) {
...
void a_callback(int, NdbTransaction *, void *) {
  ndbout_c("callback received!");
  aValue = 1;
}

int runBug44065(NDBT_Context *ctx, NDBT_Step *step) {
  /* testNdbApi -n WeirdAssertFail2
   * Results in assertion failure in DbtupCommit::execTUP_DEALLOCREQ()
   *   ndbassert(ptr->m_header_bits & Tuple_header::FREE);
   * Results in node failure
...
int runBulkPkReads(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (true) {
    ...
    if (ctx->isTestStopped() || (ctx->getProperty(ApiFailTestComplete) != 0)) {
...
int runReadColumnDuplicates(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (int m = 1; m < 100; m++) {
    ...
    if (pCon->getNdbError().code != 0) {
      ...
      ndbout << "Error at execute time : " << err.code << ":" << err.message
             << endl;
      pNdb->closeTransaction(pCon);
      result = NDBT_FAILED;
      break;
    ...
    for (int c = 0; c < (m + 1); c++) {
      if (hc.verifyRecAttr(record, 0, curr)) {
        ndbout << "Mismatch on record " << record << " column " << dupColNum
               << " read number " << c + 1 << endl;
        result = NDBT_FAILED;
        break;
      }
...
int runBug51775(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
...
int runFragmentedScanOtherApi(NDBT_Context *ctx, NDBT_Step *step) {
   ...
   */
  Uint32 *buff =
      new Uint32[PROG_WORDS + 10];  // 10 extra for final 'return' etc.
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (!trans) {
        ...
        g_err.println(
            "ERR: %u: %u: Failed to start transaction from Ndb object Error : "
            "%u %s",
      ...
      if (scan == NULL) {
        ...
        g_err.println(
            "ERR: %u: %u: Failed to get scan operation transaction Error : %u "
            "%s",
            __LINE__, stepNo, err.code, err.message);
        trans->close();
        delete[] buff;
      ...
      if (ret != 0) {
        ...
        if (execError.classification != NdbError::NodeRecoveryError) {
          g_err.println(
              "ERR: %u: %u: Execute aborted transaction with invalid error "
              "code: %u",
       ...
       * on timing
       */
      if ((execError.code != 0) && (execError.code != 874) &&
          (execError.code != 4002)) {
        g_err.println("ERR: %u: %u: incorrect error code: %u", __LINE__, stepNo,
                      execError.code);
        NDB_ERR_OUT(g_err, execError);
        trans->close();
        delete[] buff;
      ...
      if (scanError.code != 874) {
        ...
        if (scanError.classification == NdbError::NodeRecoveryError) {
          g_info.println(
              "%u: Scan failed due to node failure/disconnect with error code "
              "%u",
              stepNo, scanError.code);
          trans->close();
          break;
        } else {
          g_err.println("ERR: %u: %u: incorrect error code: %u", __LINE__,
                        stepNo, scanError.code);
...
NdbOperation *defineReadAllColsOp(HugoOperations *hugoOps,
                                  ...
                                  NdbOperation::LockMode lm, Uint32 rowNum) {
...
bool checkReadRc(HugoOperations *hugoOps, Ndb *ndb,
                 const NdbDictionary::Table *pTab, NdbOperation::LockMode lm,
...
bool checkReadDeadlocks(HugoOperations *hugoOps, Ndb *ndb,
                        const NdbDictionary::Table *pTab,
                        NdbOperation::LockMode lm, Uint32 rowNum) {
  return checkReadRc(hugoOps, ndb, pTab, lm, rowNum, 266);
}

bool checkReadSucceeds(HugoOperations *hugoOps, Ndb *ndb,
                       const NdbDictionary::Table *pTab,
                       NdbOperation::LockMode lm, Uint32 rowNum) {
...
int runTestUnlockBasic(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (Uint32 iter = 0; iter < iterations; iter++) {
    ...
    NdbOperation *op2 =
        defineReadAllColsOp(&hugoOps, trans2, pTab, lm2, rowNum);
     ...
     */
    CHECK(checkReadSucceeds(&hugoOps, GETNDB(step), pTab,
                            NdbOperation::LM_Exclusive, rowNum));
    ndbout << "Third transaction operation using LM_Exclusive succeeded"
           << endl;
...
int runTestUnlockRepeat(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (Uint32 outerLoop = 0; outerLoop < outerLoops; outerLoop++) {
    ...
    for (Uint32 iter = 0; iter < iterations; iter++) {
      ...
      CHECK(checkReadDeadlocks(&hugoOps, ndb, pTab,
                               chooseConflictingLockMode(lockAMode), rowNum));
      ...
      NdbOperation *tryLockBRowNum2 =
          defineReadAllColsOp(&hugoOps, transB, pTab, lockBMode, rowNum);
      ...
      NdbOperation::LockMode lockBOtherMode =
          chooseConflictingLockMode(lockAOtherMode);

      /* Now transaction B attempts to gain a lock on OtherRowNum
       * which should fail as transaction A still has it locked
       */
      NdbOperation *tryLockBOtherRowNum = defineReadAllColsOp(
          &hugoOps, transB, pTab, lockBOtherMode, otherRowNum);
      ...
      ndbout << "    TransB : Failed to get lock on otherRowNum as expected"
             << endl;
...
int runTestUnlockMulti(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  ndbout << "NumLocksInTransA " << numLocksInTransA << " NumLocksInTransB "
         << numLocksInTransB << " useExclusiveInA " << useExclusiveInA << endl;
  ...
  for (Uint32 depth = 0; depth < maxLocks; depth++) {
    ...
    CHECK(checkReadDeadlocks(&hugoOps, ndb, pTab, lmC, rowNum));
...
int runTestUnlockScan(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  */
  ...
  ndbout << "numTakeovers is " << numTakeovers << " takeoverMod is "
         << takeoverMod << endl;

  for (int iter = 0; iter < iterations; iter++) {
    ndbout << "Scanning table with lock mode : ";
    NdbOperation::LockMode lmScan =
        chooseLockMode(true);  // LM_Exclusive or LM_Read
    ...
    for (int lockedRows = 0; lockedRows < rowsTakenover; lockedRows++) {
      ...
      const NdbLockHandle *lockHandle =
          takeoverOps[lockedRows]->getLockHandle();
    ...
    CHECKN(otherScan->readTuples(NdbOperation::LM_Exclusive) == 0, otherScan,
           NDBT_FAILED);
...
class NodeIdReservations {
 ...
 public:
  ...
  class Reserve {
   ...
   public:
    Reserve(NodeIdReservations &res, unsigned id) : m_id(id), m_res(res) {
      m_res.lock(m_id);
    }
...
int runNdbClusterConnect(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  const Uint32 timeout_after_first_alive =
      ctx->getProperty("TimeoutAfterFirst", 30);
  ...
  while (l < loops && !ctx->isTestStopped()) {
    ...
    if (con.connect(retries, retry_delay, verbose) != 0) {
      g_err << "Ndb_cluster_connection.connect failed" << endl;
      g_err << "Error code: " << con.get_latest_error()
            << " message: " << con.get_latest_error_msg() << endl;
      return NDBT_FAILED;
    }
    ...
    if (!(ret == 0 || (timeout_after_first_alive == 0 && ret > 0))) {
      g_err << "Cluster connection was not ready, nodeid: " << con.node_id()
            << endl;
      g_err << "Error code: " << con.get_latest_error()
            << " message: " << con.get_latest_error_msg() << endl;
      abort();
...
int runRestarts(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  if (sr == 0 && restarter.getNumDbNodes() < 2) return NDBT_OK;

  while (ctx->getProperty("runNdbClusterConnect_FINISHED") < threads &&
         !ctx->isTestStopped()) {
    ndbout_c("%u %u", ctx->getProperty("runNdbClusterConnect_FINISHED"),
             threads);
    if (sr == 0) {
      ...
      ndbout << "Restart node " << nodeId
             << "(master: " << restarter.getMasterNodeId() << ")" << endl;
      ...
      if (slowNoStart) {
        ...
        while (ctx->getProperty("runNdbClusterConnect_FINISHED") < threads &&
               !ctx->isTestStopped() && NdbTick_CurrentMillisecond() < end) {
          NdbSleep_MilliSleep(100);
        }
      }
...
static bool check_connect_no_such_host() {
  for (int i = 0; i < 3; i++) {
    ...
    if (res != 1) {
      g_err << "Ndb_cluster_connection.connect(" << i << "," << i << ", 1) to '"
            << no_such_host << "' returned " << res << " instead of expected 1"
            << endl;
      return false;
    }
    g_info << "Ndb_cluster_connection.connect(" << i << "," << i << ", 1) to '"
           << no_such_host << "' returned " << res << " and message '"
           << con.get_latest_error_msg() << "'" << endl;
  }
  return true;
}

static bool check_connect_until_no_more_nodeid(const char *constr) {
  bool result = true;
...
class BasicSectionStore : public SectionStore {
 public:
  BasicSectionStore() { init(); }

  ~BasicSectionStore() override { freeStorage(); }
...
class FragmentAssembler {
 public:
      ...
      switch (sigHead->m_fragmentInfo) {
        ...
        case 3: {
          ...
          if (sigHead->m_fragmentInfo == 3) {
            ...
            sh.m_noOfSections = ((secsReceived & 4) ? 1 : 0) +
                                ((secsReceived & 2) ? 1 : 0) +
                                ((secsReceived & 1) ? 1 : 0);
            sh.m_fragmentInfo = 0;
  ...
  int handleSignal(NdbApiSignal *signal, LinearSectionPtr *sections) {
 ...
 private:
  int handleFragmentSections(const SignalHeader *sigHead, const Uint32 *sigBody,
...
static const Uint32 MAX_SEND_BYTES =
    32768; /* Align with TransporterDefinitions.hpp */
...
static const Uint32 MAX_SEGS_PER_SEND =
    (MAX_SEND_BYTES / SEGMENT_BYTES) - 2; /* Align with TransporterFacade.cpp */
        ...
        if (fa.isComplete()) {
          ...
          for (Uint32 i = 0; i < 3; i++) {
            if (bss.ptrs[i].sz != expectedSz[i]) {
              g_err << "Wrong size for section : " << i << " expected "
                    << expectedSz[i] << " but received " << bss.ptrs[i].sz
                    << endl;
              return -1;
            }
...
static int runReceiveTRANSIDAIAfterRollback(NDBT_Context *ctx,
                                            NDBT_Step *step) {
...
int testNdbRecordSpecificationCompatibility(NDBT_Context *ctx,
                                            NDBT_Step *step) {
  ...
  for (int attrId = 0; attrId < numCols; attrId++) {
    ...
    rs.nullbit_bit_in_byte = 0;
    CHECK(NdbDictionary::getOffset(defaultRecord, attrId, rs.offset));
    CHECK(NdbDictionary::getNullBitOffset(
        defaultRecord, attrId, rs.nullbit_byte_offset, rs.nullbit_bit_in_byte));
  }
  const NdbRecord *tabRec = pNdb->getDictionary()->createRecord(
      pTab, (NdbDictionary::RecordSpecification *)rsArray, numCols,
  ...
  for (int record = 0; record < numRecords; record++) {
    ...
    const NdbOperation *op = NULL;
    op = trans->insertTuple(tabRec, keyRowBuf);
    CHECK(op != 0);
    ...
    /* Verify the values read back */
    for (int col = 0; col < pTab->getNoOfColumns(); col++) {
      const char *valPtr = NdbDictionary::getValuePtr(tabRec, attrRowBuf, col);
      CHECK(valPtr != NULL);
...
int testSchemaObjectOwnerCheck(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  do {
    ...
    const NdbDictionary::Table *otherTable =
        otherNdb->getDictionary()->getTable("SchemaObjOwnerCheck_tab");
    if (!otherTable) {
    ...
    const NdbDictionary::Index *otherIndex =
        otherNdb->getDictionary()->getIndex("SchemaObjOwnerCheck_idx",
                                            "SchemaObjOwnerCheck_tab");
    if (!otherIndex) {
    ...
    if (err1.code != 1231) {
      ndbout << "Failed to detect Table with wrong owner for NdbOperation"
             << endl;
      result = NDBT_FAILED;
      break;
    }
    ...
    if (err2.code != 1231) {
      ndbout << "Failed to detect Table with wrong owner for NdbScanOperation"
             << endl;
      result = NDBT_FAILED;
      break;
    }
    ...
    if (err4.code != 1231) {
      ndbout << "Failed to detect Index with wrong owner for NdbIndexOperation"
             << endl;
      result = NDBT_FAILED;
      break;
    }
    ...
    const NdbDictionary::Table *table =
        ndb->getDictionary()->getTable("SchemaObjOwnerCheck_tab");
    if (!table) {
      ndbout << "Failed to get Ndb table from connection" << endl;
      result = NDBT_FAILED;
      break;
    }
    const NdbDictionary::Index *index = ndb->getDictionary()->getIndex(
        "SchemaObjOwnerCheck_idx", "SchemaObjOwnerCheck_tab");
    if (!index) {
    ...
    NdbIndexScanOperation *idxscanop2 =
        trans->getNdbIndexScanOperation(index, table);
    NdbIndexOperation *idxop2 = trans->getNdbIndexOperation(index, table);

    if (!op2 || !scanop2 || !idxscanop2 ||
        !idxop2)  // failure to acquire at least one op
    {
      ndbout << "Failed to acquire ";
      if (!op) ndbout << "NdbOperation, ";
      if (!scanop) ndbout << "NdbScanOperation, ";
  ...
  while (count--) {
    ...
    if (state == NULL) {
      ndbout << "ndb_mgm_get_status failed"
             ...
             << " - " << ndb_mgm_get_latest_error_msg(restarter.handle) << endl;
      result = NDBT_FAILED;
      break;
    }
 ...
 * */
int createUniqueIndex(NdbDictionary::Dictionary *pDict, const char *tableName,
                      const char *indexName, const char *columnName) {
  ...
  ndbout << "Creating index " << indexName << " on " << tableName << endl;
 ...
 */
int runTransactionUsingNdbIndexOperation(
    Ndb *pNdb, Vector<const NdbDictionary::Index *> pIndexes,
...
int runGetNdbIndexOperationTest(NDBT_Context *ctx, NDBT_Step *step) {
  /**
  ...
  CHECKN((pIndex = pDict->getIndex(indexName, tableName)) != NULL, pDict,
         NDBT_FAILED);
...
int runCreateIndexesOnI3(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (uint i = 0; i < numOfIndexes; i++) {
    ...
    CHECK(createUniqueIndex(pDict, tableName, name.c_str(), columnNames[i]) !=
          NDBT_FAILED);
  }
  return NDBT_OK;
}

int runGetNdbIndexOperationBatchTest(NDBT_Context *ctx, NDBT_Step *step) {
  /**
  ...
  for (uint i = 0; i < numOfIndexes; i++) {
    ...
    const NdbDictionary::Index *pIndex;
    name.assfmt("I3$NDBT_UIDX%d", i);
    CHECKN((pIndex = pDict->getIndex(name.c_str(), tableName)) != NULL, pDict,
           NDBT_FAILED);
    pIndexes.push_back(pIndex);
  }
  ...
  for (uint i = 0; i < numOfIndexes; i++) {
    /* drop ith index */
    ndbout << "Dropping index " << pIndexes[i]->getName() << " from "
           << tableName << endl;
...
int runGetNdbIndexOperationTransactions(NDBT_Context *ctx, NDBT_Step *step) {
  /**
   * 1. In a loop, use all the indexes to perform batch transactions
  ...
  while (ctx->getProperty("StopTransactions") == 0) {
    ...
    if (l++ % 50 == 0) {
      ...
      for (uint i = 0; i < numOfIndexes; i++) {
        ...
        if (pIndex != NULL) pIndexes.push_back(pIndex);
      }
    }
    ...
    */
    ...
    if (result != NDBT_OK && result != 241 && result != 284 && result != 283 &&
      ...
      ndberr << "Transaction failed with an unexpected error : " << result
             << endl;
      return NDBT_FAILED;
    }
  }
...
int runDropIndexesOnI3(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  const char *tableName = "I3";
  NdbDictionary::Dictionary *pDict = pNdb->getDictionary();
  const uint numOfIndexes = ctx->getProperty("numOfIndexes");
  ...
  while (loops-- > 0) {
    for (uint i = 0; i < numOfIndexes; i++) {
      ...
      name.assfmt("I3$NDBT_UIDX%d", i);
      /* drop the index. */
      ndbout << "Dropping index " << name.c_str() << " from " << tableName
             << endl;
      CHECKN(pDict->dropIndex(name.c_str(), tableName) == 0, pDict,
             NDBT_FAILED);
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    switch (testcase) {
      ...
      case 5: {
        ...
        pCon->executeAsynchPrepare(NdbTransaction::Commit, &unusedCallback,
                                   NULL);
        if (testcase == 4) break;
...
int runCheckTransId(NDBT_Context *ctx, NDBT_Step *step) {
...
int runInitCheckTransIdMt(NDBT_Context *ctx, NDBT_Step *step) {
...
int runVerifyCheckTransIdMt(NDBT_Context *ctx, NDBT_Step *step) {
  /* Look for duplicates */
  ...
  ndbout << CheckTransIdEntries << " transaction ids of which " << contigCount
         << " are contiguous, giving " << CheckTransIdEntries - contigCount
         << " gaps." << endl;
...
int runFinaliseCheckTransIdMt(NDBT_Context *ctx, NDBT_Step *step) {
  /* Free the storage */
...
int runTestColumnNameLookupPerf(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (int c = 0; c < tab->getNoOfColumns(); c++) {
    ...
    ndbout_c("Col %u %s : %u iterations in %llu millis", c, name, iterations,
             time);
  }
...
int runMaybeRestartMaster(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  if (restartMaster) {
    ndbout << "Restarting Master node " << masterNodeId << endl;
    ...
    ndbout << "Not restarting Master node " << masterNodeId << endl;
  }
  return NDBT_OK;
}
...
int runTestOldApiScanFinalise(NDBT_Context *ctx, NDBT_Step *step) {
   ...
   */
  {
     ...
     * sending
     */
    trans->executeAsynchPrepare(NdbTransaction::NoCommit, asyncCallback, NULL);
    ...
    ndbout_c(
        ...
        scanOp->getNdbError().code, scanOp->getNdbError().message);
...
static int reCreateTableHook(Ndb *ndb, NdbDictionary::Table &table, int when,
                             void *arg) {
  if (when == 0) {
...
int runReCreateTable(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  ...
  if (NDBT_Tables::createTable(pNdb, tabName.c_str(), false, false,
                               reCreateTableHook, ctx) != 0) {
    return NDBT_FAILED;
  }
...
int runCheckLateDisconnect(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  if (hugoTrans.loadTable(&otherNdb, 1024) != NDBT_OK) {
...
int runCheckWriteTransactionOverOtherNodeFailure(NDBT_Context *ctx,
                                                 NDBT_Step *step,
                                                 NdbRestarter &restarter,
                                                 Uint32 errorCode) {
  const NdbDictionary::Table *pTab = ctx->getTab();
  ...
  CHECKE((hugoOps.wait_async(pNdb) == NDBT_OK), hugoOps);
  ndbout_c("Ok");
...
int runPkRead2(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  CHECK(hugoOps.pkReadRecord(ndb, 0, 1, NdbOperation::LM_Exclusive, 0, true) ==
        0);
...
int testSlowConnectEnable(NDBT_Context *ctx, NDBT_Step *step) {
  /**
  ...
  do {
    ...
    if (otherNdb->getDictionary()->getTable(tabName) == NULL) {
      ndbout << "Get table failed with error " << otherNdb->getNdbError()
             << endl;
      break;
    }
...
TESTCASE("Bug44015", "Rollback insert followed by delete to get corruption") {
  STEP(runBug44015);
  STEPS(runScanReadUntilStopped, 10);
}
TESTCASE("Bug44065_org", "Rollback no-change update on top of existing data") {
  INITIALIZER(runBug44065_org);
}
TESTCASE("Bug44065", "Rollback no-change update on top of existing data") {
...
TESTCASE("Bug51775", "") { INITIALIZER(runBug51775); }
...
TESTCASE("UnlockBasic", "Check basic op unlock behaviour") {
  INITIALIZER(runLoadTable);
  STEP(runTestUnlockBasic);
  FINALIZER(runClearTable);
}
TESTCASE("UnlockRepeat", "Check repeated lock/unlock behaviour") {
  INITIALIZER(runLoadTable);
  STEP(runTestUnlockRepeat);
  FINALIZER(runClearTable);
}
TESTCASE("UnlockMulti", "Check unlock behaviour with multiple operations") {
  INITIALIZER(runLoadTable);
  STEP(runTestUnlockMulti);
  FINALIZER(runClearTable);
}
TESTCASE("UnlockScan", "Check unlock behaviour with scan lock-takeover") {
  INITIALIZER(runLoadTable);
...
TESTCASE("CheckTransIdMt", "Check transid uniqueness across multiple threads") {
...
TESTCASE("OldApiScanFinalise", "Test error during finalise behaviour") {
  VERIFIER(runTestOldApiScanFinalise);
}
TESTCASE("TestColumnNameLookupPerf", "") {
  INITIALIZER(runTestColumnNameLookupPerf);
}
TESTCASE("CheckDisconnectCommit", "Check commit post API disconnect") {
...
TESTCASE("CheckDisconnectComplete", "Check complete post API disconnect") {
  TC_PROPERTY("CreateRB", Uint32(1));      // ReadBackup
...
TESTCASE("CheckSlowCommit", "Check slow commit protocol + table types") {
