...
class NdbTransaction {
 ...
 public:
  ...
  /**
   * Enable/disable schema object ownership check: check that objects used by
   * this transaction belong to the dictionary owned by this connection. This is
   * a debugging feature for when multiple cluster connections are in use, and
   * has a performance cost.
   */
  void setSchemaObjOwnerChecks(bool runChecks) {
    m_enable_schema_obj_owner_check = runChecks;
  }
...
#ifndef DOXYGEN_SHOULD_SKIP_DEPRECATED
  int execute(::ExecType execType,
              ::AbortOption abortOption = ::DefaultAbortOption, int force = 0) {
    return execute((ExecType)execType, (NdbOperation::AbortOption)abortOption,
                   force);
  }
#endif
#else
  ...
  int execute(NdbTransaction::ExecType execType,
    ...
    int ret = execute((ExecType)execType,
                      (NdbOperation::AbortOption)abortOption, force);
    if (ret || (abortOption != AO_IgnoreError && theError.code)) return -1;
    return 0;
  }
#endif
   ...
   */
#ifndef NDBAPI_50_COMPAT
  void executeAsynchPrepare(
      ExecType execType, NdbAsynchCallback callback, void *anyObject,
      NdbOperation::AbortOption = NdbOperation::DefaultAbortOption);
#ifndef DOXYGEN_SHOULD_SKIP_DEPRECATED
  void executeAsynchPrepare(::ExecType execType, NdbAsynchCallback callback,
                            void *anyObject,
   ...
   */
  void executeAsynchPrepare(ExecType execType, NdbAsynchCallback callback,
                            void *anyObject, NdbOperation::AbortOption);

  void executeAsynchPrepare(
      NdbTransaction::ExecType execType, NdbAsynchCallback callback,
      void *anyObject,
...
#ifndef NDBAPI_50_COMPAT
  void executeAsynch(
      ExecType aTypeOfExec, NdbAsynchCallback aCallback, void *anyObject,
      NdbOperation::AbortOption = NdbOperation::DefaultAbortOption,
      int forceSend = 0);
#ifndef DOXYGEN_SHOULD_SKIP_DEPRECATED
  void executeAsynch(::ExecType aTypeOfExec, NdbAsynchCallback aCallback,
                     void *anyObject,
   ...
   * to be backwards compatible with previous definitions of the
   * OperationOptions structure.  If an unusual size is detected by the
   * interface implementation, it can use this to determine how to interpret the
   * passed OperationOptions structure.  To enable this functionality, the
   * caller should pass sizeof(NdbOperation::OperationOptions) for this
   * argument.
   */
...
#ifndef DOXYGEN_SHOULD_SKIP_INTERNAL
  const NdbOperation *refreshTuple(
      const NdbRecord *key_rec, const char *key_row,
      const NdbOperation::OperationOptions *opts = nullptr,
      Uint32 sizeOfOptions = 0);
#endif
   ...
   * must be created from the underlying table, not from the index to be
   * scanned.
   *
   ...
   */
  NdbQuery *createQuery(
      const NdbQueryDef *query, const NdbQueryParamValue paramValue[] = nullptr,
 ...
 private:
  ...
  static void sendTC_COMMIT_ACK(class NdbImpl *, NdbApiSignal *,
                                ...
                                Uint32 aBlockRef, bool send_immediate);
  ...
  bool checkSchemaObjects(const NdbTableImpl *tab,
                          const NdbIndexImpl *idx = nullptr);
  ...
  NdbQueryImpl
      *m_firstActiveQuery;  // First query actively executing, or completed
...
inline Uint32 NdbTransaction::get_send_size() { return 0; }
...
inline Uint32 NdbTransaction::getMagicNumberFromObject() const {
...
************************************************************************************************/
inline void NdbTransaction::setTransactionId(Uint64 aTransactionId) {
  theTransactionId = aTransactionId;
}

inline void NdbTransaction::setConnectedNodeId(Uint32 aNode,
                                               Uint32 aSequenceNo) {
  theDBnode = aNode;
...
******************************************************************************/
inline Uint32 NdbTransaction::getConnectedNodeId() { return theDBnode; }
/******************************************************************************
...
******************************************************************************/
inline void NdbTransaction::setMyBlockReference(int aBlockRef) {
  theMyRef = aBlockRef;
}
/******************************************************************************
...
******************************************************************************/
inline void NdbTransaction::setTC_ConnectPtr(Uint32 aTCConPtr) {
...
******************************************************************************/
inline int NdbTransaction::getTC_ConnectPtr() { return theTCConPtr; }

inline void NdbTransaction::setBuddyConPtr(Uint32 aBuddyConPtr) {
  theBuddyConPtr = aBuddyConPtr;
}

inline Uint32 NdbTransaction::getBuddyConPtr() { return theBuddyConPtr; }
...
******************************************************************************/
inline NdbTransaction *NdbTransaction::next() { return theNext; }
...
******************************************************************************/
inline void NdbTransaction::next(NdbTransaction *aTransaction) {
...
******************************************************************************/
inline NdbTransaction::ConStatusType NdbTransaction::Status() {
  return theStatus;
}
...
Remark:         Sets Connect status.
******************************************************************************/
inline void NdbTransaction::Status(ConStatusType aStatus) {
  theStatus = aStatus;
}
...
******************************************************************************/
inline void NdbTransaction::OpSent() { theNoOfOpSent++; }

/******************************************************************************
void executePendingBlobOps();
******************************************************************************/
inline int NdbTransaction::executePendingBlobOps(Uint8 flags) {
...
inline Uint32 NdbTransaction::ptr2int() { return theId; }
